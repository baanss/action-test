...
describe("POST /update-logs", () => {
  let agent: supertest.SuperAgentTest;

  const duplicateFilePath = "__test__/dummy/update-log_v1.1.0.0.txt";
  const newFilePath = "__test__/dummy/update-log_v10.0.0.0.txt";
  ...
  test("401 response, 헤더에 유효한 인증 정보가 없음", async () => {
    // when-then
    supertest.agent(app.getHttpServer()).post("/update-logs").expect(401);
  });
  ...
  describe("일반 계정 요청", () => {
    ...
    test("UNAUTHORIZED_AUTH_TOKEN, h-Space 요청이 아님", async () => {
      // given
      // when
      const res = await agent.post("/update-logs").set("Content-Type", "multipart/form-data").attach("file", newFilePath);
  ...
  describe("h-Space 요청", () => {
    ...
    test("201 repsonse, 이미 등록되어 있는 버전", async () => {
      // given
      const prevUpdateLog = await updateLogRepository.save({ filePath: duplicateFilePath, fileName: path.basename(duplicateFilePath), fileSize: 10 });
      const attachFile = duplicateFilePath;

      // when
      const res = await agent.post("/update-logs").set("Content-Type", "multipart/form-data").attach("file", attachFile).expect(201);

      // then: 응답이 적절함
      const expectedResult = { id: expect.any(Number) };
      expect(res.body).toEqual(expectedResult);
      // then: 이전 updateLog가 삭제된 상태로 저장됨
      const deletedUpdateLog = await updateLogRepository.findOne(prevUpdateLog.id);
      expect(deletedUpdateLog.filePath).toBeNull();
      expect(deletedUpdateLog.fileSize).toBe(0);
    });

    test("201 response, 처음 등록하는 버전", async () => {
      // given
      const prevUpdateLog = await updateLogRepository.save({ filePath: newFilePath, fileName: path.basename(newFilePath), fileSize: 10 });
      ...
      const deletedUpdateLog = await updateLogRepository.findOne(prevUpdateLog.id);
      expect(deletedUpdateLog.filePath).toBeNull();
      expect(deletedUpdateLog.fileSize).toEqual(0);
    });
  });
});
